...
class MvcAdminLoader extends MvcLoader
{
    ...
    public function add_menu_pages_old()
    {
        ...
        //        wp_die( '<pre>' . var_export( $this->admin_controller_names, true ) . '</pre>' );
    ...
    public function add_menu_pages()
    {
        ...
        $admin_pages = MvcConfiguration::get('AdminPages');
        $single_root_page = MvcConfiguration::get('SingleRootMenu');
        $is_nested_menu = !empty($single_root_page);

        if ($is_nested_menu) {
            $this->add_root_menu_page($single_root_page);
        }
        foreach ($this->admin_controller_names as $controller_name) {
    ...
    public function add_root_menu_page($root_page)
    {
        ...
        $default_root_page['function'] = $default_root_page['controller'] ? array($this->dispatcher, 'admin_' . $controller . '_' . $action) : '';

        add_menu_page(
            $default_root_page['page_title'],
            $default_root_page['menu_title'],
            $default_root_page['capability'],
            $default_root_page['menu_slug'],
            $default_root_page['function'],
            $default_root_page['icon_url'],
            $default_root_page['position']
        );
    }
